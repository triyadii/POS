...
class LaporanPenjualanBrandController extends Controller
{
    ...
    public function getLaporanData(Request $request)
    {
        ...
        return \DataTables::of($data)
            ...
            // KODE BARU
            ->addColumn('action', function ($data) use ($brandId) {
                // 1. Filter detail HANYA untuk brand yang relevan jika ada filter
                $filteredDetails = $data->detail->when($brandId, function ($collection) use ($brandId) {
                ...
                $button = '<button type="button" class="btn btn-sm btn-light-primary" 
                    ...
                    data-bs-target="#modal_detail_penjualan_brand" 
