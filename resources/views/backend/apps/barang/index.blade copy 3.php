                                  ...
                                  <div data-repeater-item class="border rounded p-5 mb-5 bg-light">
                                    ...
                                    <div class="row g-3 mb-4">
                                      ...
                                      <div class="col-md-3">
                                        ...
                                        <select data-type="kategori" name="kategori_id" class="form-select form-select-sm kategori_id" data-kt-repeater="select2" data-placeholder="pilih kategori"></select>
                                        <span class="text-danger error-text kategori_id_error_add"></span>
                                      </div>
        ...
        <div class="row g-3 align-items-start">
          <div class="col-md-2">
            <select data-type="tipe" name="tipe_id" class="form-select form-select-sm tipe_id"
              ...
              <span class="text-danger error-text tipe_id_error_add"></span>

          </div>
  
          <!-- Kolom Kode Item + Sub Repeater -->
          <div class="col-md-2">
            <input type="text" name="kode" class="form-control form-control-sm mb-2 kode" placeholder="Kode Item">
            <span class="text-danger error-text kode_error_add"></span>
            ...
            <div class="sub-repeater ms-2">
              <div data-repeater-list="variasi">
                <div data-repeater-item class="row g-2 align-items-center mb-2">
                  <div class="col-7">
                    <input type="text" name="kode_variasi" id="kode_variasi" class="form-control form-control-sm kode_variasi"
                      placeholder="Kode Item">
                      <span class="text-danger error-text kode_variasi_error_add"></span>

                  </div>
                  <div class="col-4">
                    <input type="text" name="size" class="form-control form-control-sm size" placeholder="Size">
                    <span class="text-danger error-text size_error_add"></span>
          ...
          <div class="col-md-2">
            <input type="text" name="nama" class="form-control form-control-sm nama" placeholder="Nama Item">
            <span class="text-danger error-text nama_error_add"></span>

          </div>
          <div class="col-md-1">
            <input type="text" name="size_main" class="form-control form-control-sm size_main" placeholder="Size">
            <span class="text-danger error-text size_main_error_add"></span>
          ...
          <div class="col-md-1">
            <select data-type="satuan" name="satuan_id" class="form-select form-select-sm satuan_id"
              ...
              <span class="text-danger error-text satuan_id_error_add"></span>

          </div>
          <div class="col-md-1">
            <input type="text" name="harga_jual" class="form-control form-control-sm text-end format-rupiah harga_jual"
              placeholder="Harga Jual">
              <span class="text-danger error-text harga_jual_error_add"></span>

          </div>
          <div class="col-md-1">
            <input type="text" name="harga_beli" class="form-control form-control-sm text-end format-rupiah harga_beli"
              placeholder="Harga Beli">
              <span class="text-danger error-text harga_beli_error_add"></span>
    ...
    $(this).val(new Intl.NumberFormat('id-ID').format(val));
                ...
                $('#btn_tambah_data').click(function() {
                    $('#Modal_Tambah_Data').modal('show');
                                ...
                                // Setelah SweetAlert (atau di bawah setTimeout)
$(document).on('input change', 'input, select, textarea', function () {
    const $field = $(this);
    const name = $field.attr('name'); // contoh: nama, kode, harga_beli
    if (!name) return;

    // Bersihkan error untuk field ini
    $field.removeClass('is-invalid');
    $('span.' + name + '_error_add').text('');
});
                ...
                $(document).on('keypress', 'input[name="kode"], input[name="kode_variasi"]', function(e) {
                    if (e.which === 13) {
                        e.preventDefault();
                        // bisa tambahkan logic khusus, misalnya pindah ke field berikutnya:
...
// === Bersihkan pesan error otomatis saat user isi ulang ===
$(document).on('input change', 'input, select, textarea', function () {
    const $field = $(this);
    const rawName = $field.attr('name');
    if (!rawName) return;

    // Ambil nama field terakhir (hilangkan array index)
    const name = rawName.split('[').pop().replace(']', '');

    // Hilangkan style & pesan error
    $field.removeClass('is-invalid');
    $('span.' + name + '_error_add').text('');
});

// === Tambahkan juga untuk Select2 ===
                ...
                $('#FormTambahModalID').on('submit', function(event) {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            if (result.errors) {
//                                 setTimeout(function () {
//     // Reset semua pesan error & border merah
//     $('.error-text').text('');
//     $('.is-invalid').removeClass('is-invalid');

//     // Loop semua error dari backend
//     $.each(result.errors, function (prefix, val) {
//         // Ambil field terakhir dari key validasi (misal: "nama" dari "kelompok_barang.0.barang.1.nama")
//         const field = prefix.split('.').pop();

//         // Tampilkan pesan error ke semua elemen yang punya class tersebut
//         $('span.' + field + '_error_add').text(val[0]);

//         // Tambahkan border merah di input/select dengan name yang sama
...
setTimeout(function () {
    ...
    $.each(result.errors, function (key, val) {
        // Ambil nama field terakhir dari key validasi
        const field = key.split('.').pop();
        const message = val[0];

        // Tampilkan di semua span error dengan class [field]_error_add
        $('span.' + field + '_error_add').text(message);

        // Tambahkan border merah ke semua input/select dengan name yang sama
        $('[name="' + field + '"]').addClass('is-invalid');
    });
