...
function cetakStruk(payload) {
    ...
    const tanggalCetak = new Date(payload.tanggal_raw || payload.tanggal).toLocaleString('id-ID', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric',
        hour: '2-digit',
    ...
    payload.items.forEach(i => {
        ...
        const harga = numToId(i.harga_jual);
        const subtotal = numToId(i.subtotal);
        itemRows += `
          <tr>
    ...
    const strukHTML = `
      <div style="font-family: monospace; font-size: 12px; width: 260px; margin: auto;">
        ...
        <div style="line-height:1.4;">
            ...
            <div>Kode    : ${payload.no_penjualan}</div>
        ...
        <hr style="border-top:1px dashed #000;">
        <div style="display:flex;justify-content:space-between;"><span>Potongan</span><span>Rp ${numToId(payload.potongan)}</span></div>
        <div style="display:flex;justify-content:space-between;"><span>Total</span><span>${payload.total ?? '-'}</span></div>
        <div style="display:flex;justify-content:space-between;"><span>Bayar</span><span>${payload.uang ?? '-'}</span></div>
        <div style="display:flex;justify-content:space-between;"><span>Kembalian</span><span>${payload.kembalian ?? '-'}</span></div>
        <div style="display:flex;justify-content:space-between;"><span>Metode</span><span>${payload.pembayaran_nama ?? '-'}</span></div>
        <hr style="border-top:1px dashed #000;">
    ...
    w.document.write(`
      ...
      <head>
        ...
        <style>
          body { font-family: monospace; font-size: 11px; margin: 0; padding: 5px 8px; width: 58mm; color:#000; }
          ...
          @media print {
            ...
            body::after { content: ""; display:block; height: 50mm; }
          }
        </style>
      </head>
      <body>${strukHTML}</body>
    ...
    w.onload = function() {
        ...
        try {
            ...
            console.warn('⚠️ Auto print gagal:', err);
        }
        setTimeout(() => {
            w.close();
        }, 500);
...
$(document).ready(function() {
                ...
                tbody.append(`
                  <tr>
                    ...
                    <td class="text-center">
                      <button class="btn btn-sm btn-light-primary" onclick="lihatDetail('${p.id}')"><i class="fas fa-eye"></i></button>
                      <button class="btn btn-sm btn-light-success" onclick="cetakStrukHistory('${p.id}')"><i class="fas fa-print"></i></button>
                    </td>
                  </tr>`);
            });
        }).fail(() => tbody.html(
    ...
    window.cetakStrukHistory = function(id) {
        $.get("{{ route('penjualan.history.data') }}", function(res) {
            const trx = res.find(x => String(x.id) === String(id));
            if (!trx) {
            ...
            const tanggalCetak = new Date(trx.tanggal_penjualan).toLocaleString('id-ID', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric',
                hour: '2-digit',
            ...
            let itemRows = '';
            (trx.detail || []).forEach(i => {
                itemRows += `
              <tr>
                <td colspan="2">${i.barang?.nama ?? '-'}</td>
                <td style="text-align:right;">Rp ${numToId(i.harga_jual)}</td>
              </tr>
              <tr>
                <td style="width:10%;"></td>
                <td>${i.qty} x Rp ${numToId(i.harga_jual)}</td>
                <td style="text-align:right;">Rp ${numToId(i.subtotal)}</td>
              </tr>`;
            });
          ...
          <div style="font-family: monospace; font-size: 12px; width: 260px; margin: auto;">
            ...
            <div style="line-height:1.4;">
              <div>Tanggal : ${tanggalCetak}</div>
              <div>Kode    : ${trx.kode_transaksi}</div>
              <div>Kasir   : Kasir</div>
              <hr style="border-top:1px dashed #000;">
            </div>
            <table style="width:100%;border-collapse:collapse;"><tbody>${itemRows}</tbody></table>
            <hr style="border-top:1px dashed #000;">
            <div style="display:flex;justify-content:space-between;"><span>Total</span><span>Rp ${numToId(trx.total_harga)}</span></div>
            <div style="display:flex;justify-content:space-between;"><span>Pembayaran</span><span>${trx.pembayaran?.nama ?? '-'}</span></div>
            <hr style="border-top:1px dashed #000;">
            <div style="text-align:center;margin-top:5px;">Terima Kasih Atas Kunjungan Anda</div>
          </div>`;
            ...
            w.document.write(`
          <html><head><title>Struk Penjualan</title>
          <style>
